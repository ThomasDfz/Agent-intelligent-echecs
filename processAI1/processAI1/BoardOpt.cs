...
namespace processAI1
{
    class BoardOpt
    {
        ...
        private ulong[] myPiecesPosition;
        private int[] myPiecesValue;
        private ulong[] advPiecesPosition;
        private int[] advPiecesValue;
        private ulong[] voidTiles;

        private ulong boardFreeTile = 0;
        private ulong boardAdvPieces = 0;
        private ulong boardMyPieces = 0;
        ...
        {
            ulong newPos = 0;
            if (getColumn(start) + col <= 0 || getColumn(start) + col > 8)
                return 0; // out of board
            if (getLine(start) + line <= 0 || getLine(start) + line > 8)
                return 0; // out of board
        ...
        {  // ?? implements change piece ??
            ...
            if (whitePlayer)
            {
                ...
                for (int i = 0; i < playsCol.Length; i++)
                {
                    ...
                    if ((newPositions[i] & boardAdvPieces) != 0)
                    {
            ...
            else
            {
                ...
                {
                    newPositions.Add(newPosition(position, playsCol[i], playsLine[i]));
                    if ((newPositions[i] & boardMyPieces) != 0)
                    {
                        solutions.Add(newPositions[i]);
                        values.Add(mapValues[newPositions[i]]);
                    }
            ...
            for (int dir = 0; dir < 4; dir++)
            {
                ...
                for (int i = 1; i <= 7; i++)
                {
                    ...
                    if (np != 0) // still in board
                    {
                        ...
                        {
                            if ((whitePlayer && (np & boardAdvPieces) != 0) || (!whitePlayer && (np & boardAdvPieces) != 0)) // on an adverse tile
                            {
                                solutions.Add(np);
        ...
        public void GetPossiblePositions(ulong posPiece, out ulong[] solutions, out int[] values)
        {
            ...
            bool whitePlayer = (typePiece < 0) ? false : true;
        ...
        /* constructor */
        public BoardOpt(string[] myPieces, string[] advPieces, string[] voidTiles, int[] myPiecesT, int[] advPiecesT, bool wp = true)
        {
            whitePlayerer = wp;
            this.myPiecesPosition = new ulong[myPieces.Length];
            int i = 0;
            foreach (string s in myPieces)
            {
                this.myPiecesPosition[i++] = ConvertPositionStringToLong(s);
            }
            this.advPiecesPosition = new ulong[advPieces.Length];
            i = 0;
            foreach (string s in advPieces)
            {
                this.advPiecesPosition[i++] = ConvertPositionStringToLong(s);
            }
            this.voidTiles = new ulong[voidTiles.Length];
            i = 0;
            foreach (string s in voidTiles)
            {
                this.voidTiles[i++] = ConvertPositionStringToLong(s);
            }
            ...
            int n = 0;
            foreach (ulong pos in this.myPiecesPosition)
            {
                this.boardMyPieces = this.boardMyPieces | pos;
                mapValues[pos] = typePieceToValue(myPiecesValue[n]);
                mapTypes[pos] = myPiecesValue[n];
                n++;
            }
            n = 0;
            foreach (ulong pos in this.advPiecesPosition)
            {
                this.boardAdvPieces = this.boardAdvPieces | pos;
                mapValues[pos] = typePieceToValue(advPiecesValue[n]);
                mapTypes[pos] = advPiecesValue[n];
                n++;
            }
